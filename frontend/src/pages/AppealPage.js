  ...
  return (
    <div className="min-h-screen bg-gray-200 text-gray-900 font-sans">
      ...
      <main className="p-4 sm:p-6 md:p-8">
        <h1 className="text-xl sm:text-2xl md:text-3xl font-bold text-center mb-4 sm:mb-6 md:mb-8">{t.rejectedCases}</h1>
        ...
        {rejectedCases.length === 0 ? (
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-1 gap-3 sm:gap-4 md:gap-6 max-w-4xl mx-auto">
            {rejectedCases.map(({ id, fullName, caseType, caseDescription, status }) => (
              <div key={id} className="bg-white p-3 sm:p-4 md:p-6 rounded-xl shadow">
                <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-1 sm:gap-2">
                  <div>
                    <h2 className="text-base sm:text-lg md:text-xl font-semibold">{fullName}</h2>
                    <p className="text-xs sm:text-sm text-gray-600">{caseType}</p>
                  </div>
                  <p className="text-xs sm:text-sm font-bold text-red-600">
                    {t.caseStatus[status.toLowerCase()] || status}
                  </p>
                </div>
                <p className="text-xs sm:text-sm text-gray-700 mt-1 sm:mt-2 md:mt-3">{caseDescription}</p>

                <div className="mt-2 sm:mt-3 md:mt-4 flex justify-end">
                  <button
                    // onClick={() => navigate(`/appealform/${id}`)}
                    className="bg-[#f25c05] hover:bg-[#d14e00] text-white px-3 py-1 sm:px-4 sm:py-2 rounded-full font-semibold transition text-xs sm:text-sm md:text-base"
                  >
                    {t.appealButton}
                  </button>
                </div>
